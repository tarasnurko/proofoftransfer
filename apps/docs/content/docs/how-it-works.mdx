---
title: How It Works
description: Technical deep dive into nullifiers, merkle trees, ZK proofs, EIP-712 signing, and architecture
---
...
Transfers are not linked to claims via a join table. Instead, they are queried at runtime by matching the claim's constraints and sorted by `blockTimestamp`. This works because transfers are immutable on-chain — the same query always returns the same results.
