...
1. Server fetches all transfers for the claim, builds the full merkle tree, and computes merkle inclusion proofs for each transfer
2. Client receives all transfers and merkle proofs, then filters to the prover's transfers locally by matching wallet address against sender/recipient. The wallet address is never sent to the server
...
6. Client assembles circuit inputs: claim fields, prover's padded transfers, merkle proofs, signature components, and public key
